        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
                ...
                <div class="hidden md:block">
                    <img src="" alt="Shopping" class="w-full">
                </div>
            </div>
        </div>
    </section>
                ...
                @forelse($categories as $category)
                    <a href="" class="group">
                        <div class="bg-white dark:bg-gray-800 rounded-xl p-6 text-center hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-500">
                            @if($category->image)
                                <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->name }}" class="w-20 h-20 mx-auto mb-4 rounded-lg object-cover">
                            @else
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $category->products_count ?? 0 }} items</p>
                        </div>
                    </a>
                @empty
                ...
                @forelse($featuredProducts as $product)
                    <div class="bg-white dark:bg-gray-900 rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 group">
                        ...
                        <div class="relative overflow-hidden aspect-square">
                            @if($product->images)
                                <img src="{{ asset('storage/uploads/product/' . $product->images) }}" alt="{{ $product->name }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-300">
                            @else
                            ...
                            @if($product->discount > 0)
                                <span class="absolute top-3 right-3 bg-red-500 text-white px-3 py-1 rounded-full text-sm font-bold">
                                    -{{ $product->discount }}%
                                </span>
                            @endif
                        ...
                        <div class="p-4">
                            ...
                            <div class="flex items-center mb-3">
                                ...
                                <span class="text-xs text-gray-500 dark:text-gray-400 ml-2">({{ rand(10, 100) }})</span>
                            </div>
                            ...
                            <div class="flex items-center justify-between mb-3">
                                <div>
                                    @if($product->discount > 0)
                                        <p class="text-lg font-bold text-blue-600 dark:text-blue-400">${{ number_format($product->sale_price, 2) }}</p>
